...
def match(img: NDArray, template: NDArray):
    ...
    img_match = cv2.drawMatches(img, key_points2, template, key_points1, good[:100],
                                None, flags=2)
    src_points = np.float32([key_points2[m.queryIdx].pt for m in good]).reshape(-1, 1,
                                                                                2)
    dst_points = np.float32([key_points1[m.trainIdx].pt for m in good]).reshape(-1, 1,
                                                                                2)
...
def ocr(filepath: str, templatepath: str):
    ...
    for x, r in enumerate(roi):
        ...
        thresh = \
            cv2.threshold(img_crop, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
